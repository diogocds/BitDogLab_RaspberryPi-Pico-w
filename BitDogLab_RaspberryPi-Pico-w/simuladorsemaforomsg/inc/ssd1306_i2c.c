...
{
  uint8_t *temp_buffer = (uint8_t *)malloc(buffer_length + 1); // Aloca dinamicamente o buffer
  if (temp_buffer == NULL)
  {
  ...
  free(temp_buffer); // Libera o buffer alocado após o uso
}
...
void ssd1306_draw_string(uint8_t *ssd, int16_t x, int16_t y, char *string)
{
  ...
  // Desenha até 3 linhas
  int line = 0;
  while (*string && line < 3) // Vamos permitir até 3 linhas
  {
    // Desenha a linha até encontrar um '\n' ou o fim da string
    while (*string && *string != '\n' && line < 3)
    {
      ssd1306_draw_char(ssd, x, y, *string++);
      x += 8; // Move para o próximo caractere na linha
    }
    ...
    {
      string++; // Pula o '\n'
      x = 0;    // Reseta a posição x para o início da linha
      y += 8;   // Desloca a posição y para a próxima linha
    }
    else
    {
      // Caso a string tenha acabado ou não tenha mais '\n', pula para a próxima linha
      x = 0;
      y += 8;
    }

    line++; // Incrementa a contagem das linhas
  }
}
...
// Desenha o bitmap (a ser fornecido em simuladorsemaforomsg.c) no display
